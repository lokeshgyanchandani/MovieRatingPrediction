...
def euclidean_similarity(prefs, movie1, movie2):
        # Get the list of the commonly rated movies
        commonMovies = {}

        # Find the common movies and them to the dictionary
        for user in prefs[movie1]:
                if user in prefs[movie2]:
                        commonMovies[user] = 1
        ...
        distance = sqrt(sum([pow((prefs[movie1][user] - prefs[movie2][user]), 2) for user in commonMovies]))
...
def getRecommendedItems(prefs, itemMatch, user):
        ...
        rankings=[(score/1+totalSim[item],item) for item,score in scores.items()]
...
if __name__ == "__main__":
        ...
        print ("Calculating top similar items for each item")
